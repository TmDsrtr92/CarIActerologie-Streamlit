"""
Chunks renderer service - handles display of retrieved document chunks.
Moved from utils/chunks_display.py into UI service for proper architectural separation.
"""
...
class ChunksRenderer:
    """
    Service for rendering retrieved document chunks in the UI.
    Handles both full and simple chunk display components.
    """
    
    def __init__(self):
        self.logger = get_logger(__name__)
    
    def render_chunks_component(self, documents: List[Document], question: str = "") -> None:
        """
    ...
    def render_simple_chunks_list(self, documents: List[Document]) -> None:
        """
...
class ChunksCollector:
    ...
    Maintains state and coordinates with ChunksRenderer for display.
    """
    
    def __init__(self, renderer: Optional[ChunksRenderer] = None):
        self.chunks: List[Document] = []
        self.question: str = ""
        self.renderer = renderer or ChunksRenderer()
        self.logger = get_logger(__name__)
    
    def set_question(self, question: str):
        """Set the current question"""
        self.question = question
        self.logger.debug(f"Set chunks question: {question[:50]}...")
    
    def add_chunks(self, documents: List[Document]):
        """Add retrieved chunks"""
        self.chunks = documents.copy()
        self.logger.debug(f"Added {len(documents)} chunks to collector")
    
    def clear(self):
        """Clear stored chunks and question"""
        self.chunks = []
        self.question = ""
        self.logger.debug("Cleared chunks collector")
    
    def render_if_available(self):
        """Render chunks component if chunks are available"""
        if self.chunks:
            self.renderer.render_chunks_component(self.chunks, self.question)
            self.logger.debug(f"Rendered {len(self.chunks)} chunks")
...
# Global chunks renderer instance
_chunks_renderer: Optional[ChunksRenderer] = None


def get_chunks_renderer() -> ChunksRenderer:
    """Get the global chunks renderer instance"""
    global _chunks_renderer
    if _chunks_renderer is None:
        _chunks_renderer = ChunksRenderer()
    return _chunks_renderer


# Legacy compatibility functions for existing imports
def render_chunks_component(documents: List[Document], question: str = "") -> None:
    """Render chunks component (legacy compatibility)"""
    renderer = get_chunks_renderer()
    renderer.render_chunks_component(documents, question)


def render_simple_chunks_list(documents: List[Document]) -> None:
    """Render simple chunks list (legacy compatibility)"""
    renderer = get_chunks_renderer()
    renderer.render_simple_chunks_list(documents)